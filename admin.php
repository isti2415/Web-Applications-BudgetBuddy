    ...
    <div class="bg-white dark:bg-gray-900 min-h-screen flex flex-col justify-center items-center">
            ...
            <div class="flex flex-wrap justify-between items-center">
                <div class="flex justify-start items-center">
                    ...
                    <a href="index.php" class="flex items-center">
                        <img src="/images/logo.png" class="h-10" alt="BudgetBuddy Logo" />
        ...
        <aside class="fixed top-0 left-0 z-40 w-64 h-screen pt-14 transition-transform -translate-x-full bg-white border-r border-gray-200 md:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidenav" id="drawer-navigation">
            <div class="overflow-y-auto py-5 px-3 h-full bg-white dark:bg-gray-800">
                <ul class="space-y-2">
                    <li>
                        <a onclick="toggleUsers()" class="cursor-pointer flex items-center p-2 text-base font-medium text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
        ...
        <section id="users-section" class="relative md:left-32">
                ...
                <div class="flex items-center justify-between pb-4 bg-white dark:bg-gray-900">
                    <div>
                        <button class="inline-flex items-center text-gray-500 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-1.5 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700" type="button">
                            Delete User
                            <svg class="pl-2 w-6 h-6 text-red-600" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 18 20">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        if (isset($_GET['search_term']) && !empty(trim($_GET['search_term']))) {
                            $search_term = mysqli_real_escape_string($connection, $_GET['search_term']);
                            $whereClause = "WHERE name LIKE '%{$search_term}%' OR email LIKE '%{$search_term}%'";
                        }
                        $query = "SELECT user_id, email, name, profile_picture FROM budgetbuddy.users $whereClause";
                        $result = mysqli_query($connection, $query);
                        while ($row = mysqli_fetch_assoc($result)) {
                            echo '<tr class="border-b dark:border-gray-900">';
                            echo '<td class="p-4">';
                            echo '<div class="flex items-center">';
                            echo '<input id="checkbox-1" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">';
                            echo '<label for="checkbox-1" class="sr-only">checkbox</label>';
                            echo '</div>';
                            echo '</td>';
                            echo '<td class="px-4 py-3">' . htmlspecialchars($row['user_id']) . '</td>';
                            echo '<td class="px-4 py-3">' . htmlspecialchars($row['email']) . '</td>';
                            echo '<td class="px-4 py-3">' . htmlspecialchars($row['name']) . '</td>';
                            echo '<td class="px-4 py-3"> <img class="w-8 h-8 rounded-full" src="' . $row['profile_picture'] . '" alt="user photo"> </td>';
                            echo '</tr>';
                        }
                        ?>
                    </tbody>
